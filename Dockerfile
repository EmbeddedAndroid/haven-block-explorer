...
RUN mkdir -p  /src \
    && git clone https://github.com/havenprotocol/haven-block-explorer.git

ARG NPROC
RUN cd /src/haven-block-explorer \
    && mkdir build \
    && cd build \
    && cmake .. -DMONERO_DIR=/src/haven \
...
COPY --from=builder /src/haven-block-explorer/build/xmrblocks /usr/local/bin/
